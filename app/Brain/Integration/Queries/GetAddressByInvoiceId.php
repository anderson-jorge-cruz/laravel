...
class GetAddressByInvoiceId extends Query
{
    ...
    public function handle(): Collection|stdClass
    {
        ...
        return ENDERECO::query()
            ...
            ')
            ->where('IDENTIDADE', function ($query) use ($invoiceID) {
                $query->select('IDENT_ENTREGA')
                    ->from('WMSPRD.NOTAFISCAL as n')
                    ->where('n.IDNOTAFISCAL', $invoiceID)
                    ->limit(1);
            })
            ->getQuery()
            ->first();
